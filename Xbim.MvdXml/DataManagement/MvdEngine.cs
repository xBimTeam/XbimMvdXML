...
[assembly: InternalsVisibleTo("Library.Test, PublicKey=")]
...
namespace Xbim.MvdXml.DataManagement
{
	...
	public partial class MvdEngine
	{
		...
		private Dictionary<ExpressType, List<ConceptRoot>> _expressTypeConceptRootLookup = new();
		...
		{
			_expressTypeConceptRootLookup = new();
			Mvd.Engine = this;
			if (Mvd.Views != null)
			{
		...
		internal DataFragment? ProcessRuleTree(IPersistEntity entity, DataIndicatorLookup dataIndicators, AttributeRule[] rules, string prefix)
		{
			...
			foreach (var attributeRule in rules)
			{
				...
				var entityRuleValue = GetFieldValue(entity, attributeRule.AttributeName, out var retProp);
				if (retProp == null) // we are in the case where the property does not exist in the schema
				{
					Log.LogWarning("{attName} property is not available for type {EntityTypeName} (as expected on attributeRule '{ruleID}' in {parentConceptId}).",
						attributeRule.AttributeName,
						entity.GetType().Name,
						attributeRule.RuleID,
						attributeRule.ParentConceptTemplate?.uuid
						);
					continue;
				}
		...
		private DataFragment? ExtractRulesValues(IPersistEntity entity, DataIndicatorLookup dataIndicators, IEnumerable<IRule> rules, string prefix)
		{
			...
			foreach (var attributeRule in rules)
			{
				...
				{
					value = GetFieldValue(entity, attributeRule.Name, out var retProp);
					if (retProp == null)
					{
				...
				{
					DataFragment tF;
					if (value is IEnumerable<object> asEnum)
					{
				...
				if (dataIndicators.Requires(storageName, DataIndicator.ValueSelectorEnum.Type))
				{
					...
					tmp.Add(new DataFragment(storName,
						value is not null
							? value.GetType().Name
							: ""
						));
				...
				if (dataIndicators.Requires(storageName, DataIndicator.ValueSelectorEnum.Exists))
				{
					...
					{
						storValue = !string.IsNullOrWhiteSpace(value.ToString());
					}
		...
		public List<ExpressType> GetExpressTypes()
		{
			...
			foreach (var classSchemaPair in classesInFilePrep)
			{
				...
				{
					Log.LogError("{schemaPair} is not a recognised class identification.", classSchemaPair);
					continue;
				}
		...
		internal DataFragment? FillEntities(EntityRule[] rules, IPersistEntity? entity, DataIndicatorLookup dataIndicators, string prefix)
		{
			...
			foreach (var entityRule in rules)
			{
				...
				if (entityRule.References != null)
				{
					...
					var refR = entityRule.References.Template?.@ref;
					...
					var refTemplate = Mvd.GetConceptTemplate(refR);
				...
				{
					var found = DataFragment.Combine(thisRuleFragments);
					if (found != null)
						retFragments.Add(found);
				}
			}
			if (retFragments.Any())
			{
		...
		private Dictionary<string, ExpressMetaData> _metadatas = new()
		{
		...
		internal Dictionary<string, ExpressMetaData> SchemaMetadatas => _metadatas;
		...
		{
			if (applicableClass is null)
				return [];
			return _expressTypeConceptRootLookup.TryGetValue(applicableClass, out var ret)
				? ret
				: [];
		}
		...
		{
			if (_expressTypeConceptRootLookup.TryGetValue(st, out var dicItem))
			{
				dicItem.Add(conceptRoot);
			}
			else
			{
				_expressTypeConceptRootLookup.Add(st, [conceptRoot]);
			}
		}
		...
		public Concept? GetConcept(string conceptUuid)
		{
			...
			_dicConcepts.TryGetValue(conceptUuid, out var cRet);
			return cRet;
		}
		...
		internal ExpressMetaData? GetSchema(string schemaIdentifier)
		{
			...
			if (SchemaMetadatas.TryGetValue(schemaIdentifier.ToLower(), out var schemaToUse))
				return schemaToUse;
